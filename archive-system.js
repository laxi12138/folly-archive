...
const i18n = {
  zh: {
    document_title:'遺構館 · 檔案系統', panel_title:'遺構館 · 檔案系統', form_title:'檔案投稿', mark_location:'標記地點', layers_title:'圖層',
        layer_border: '疆界', layer_satellite: '衛星影像', layer_terrain: '地形線', layer_human: '聚居痕跡', layer_archive: '館藏遺構', layer_visitors:'客錄遺構', layer_discovery:'发现', layer_pilgrimage:'圣地巡礼', archive_label:'歸檔：', recorder_label:'記錄者：',
  ...
  en: {
    ...
    layer_border:'Boundaries', layer_satellite:'Satellite Imagery', layer_terrain:'Terrain Lines', layer_human:'Settlement Traces', layer_archive:'Collection Relics', layer_visitors:'Guest-recorded Relics', layer_discovery:'Discovery', layer_pilgrimage:'Pilgrimage', archive_label:'Archive: ', recorder_label:'Recorder: ',
  ...
  ja: {
    ...
    layer_border:'境界', layer_satellite:'衛星画像', layer_terrain:'地形線', layer_human:'居住痕跡', layer_archive:'収蔵遺構', layer_visitors:'客録遺構', layer_discovery:'発見', layer_pilgrimage:'聖地巡礼', archive_label:'収蔵：', recorder_label:'記録者：',
...
function markerGroupControls(group) {
  ...
  return {
    ...
    discovery: document.getElementById(`layer-${prefix}-discovery`),
    pilgrimage: document.getElementById(`layer-${prefix}-pilgrimage`),
    groupEl: document.getElementById(`layer-${prefix}-group`)
  };
}

function visitModeEnabled(site) {
  const controls = markerGroupControls(markerGroupOf(site));
  return visitModeOf(site) === 'pilgrimage'
    ? Boolean(controls.pilgrimage?.checked)
    : Boolean(controls.discovery?.checked);
}
...
const markerLayerGroups = {
  archive: { ...markerGroupControls('archive'), wasEnabled: Boolean(document.getElementById('layer-archive')?.checked) },
  visitors: { ...markerGroupControls('visitors'), wasEnabled: Boolean(document.getElementById('layer-visitors')?.checked) }
};

function syncMarkerGroupAvailability(groupKey) {
  const controls = markerLayerGroups[groupKey];
  if (!controls) return;
  const enabled = Boolean(controls.parent?.checked);

  if (enabled && !controls.wasEnabled) {
    if (controls.discovery) controls.discovery.checked = true;
    if (controls.pilgrimage) controls.pilgrimage.checked = true;
  }

  for (const input of [controls.discovery, controls.pilgrimage]) {
    if (input) input.disabled = !enabled;
  }
  controls.groupEl?.classList.toggle('is-disabled', !enabled);
  controls.wasEnabled = enabled;
}

function refreshMarkerLayers() {
  syncMarkerGroupAvailability('archive');
  syncMarkerGroupAvailability('visitors');
  addExistingMarkers();
  buildVisitorMarkers();
...
for (const groupKey of ['archive', 'visitors']) {
  const controls = markerLayerGroups[groupKey];
  controls.parent?.addEventListener('change', refreshMarkerLayers);
  controls.discovery?.addEventListener('change', refreshMarkerLayers);
  controls.pilgrimage?.addEventListener('change', refreshMarkerLayers);
}
...
bindMarkerLayerPulse('layer-archive-discovery', site => !isVisitorRecord(site) && visitModeOf(site) === 'discovery');
bindMarkerLayerPulse('layer-archive-pilgrimage', site => !isVisitorRecord(site) && visitModeOf(site) === 'pilgrimage');
bindMarkerLayerPulse('layer-visitors-discovery', site => isVisitorRecord(site) && visitModeOf(site) === 'discovery');
bindMarkerLayerPulse('layer-visitors-pilgrimage', site => isVisitorRecord(site) && visitModeOf(site) === 'pilgrimage');
