...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (e) => {
        ...
        if (indexDrawer && indexDrawer.classList.contains('open')) {
            // 判断点击的目标是否在抽屉内部，或者是否点在了触发按钮上
            const isClickInsideDrawer = indexDrawer.contains(e.target);
            const isClickOnTrigger = Array.from(triggers).some(trigger => trigger.contains(e.target));

            // 如果既不在抽屉内，也没有点在触发器上，则关闭抽屉
...
map.on('zoomend', function () {
    ...
    // 仅在层级 >= 6 且 < 7 时添加光晕
    if (currentZoom >= 4) {
        mapContainer.classList.add('zoom-6-glow');
    } else {
        mapContainer.classList.remove('zoom-6-glow');
    }
});
...
if (initZoom >= 4 ) {
    document.getElementById('map').classList.add('zoom-6-glow');
}
/* ======================================================== */
...
const drawer = document.getElementById('archive-drawer');
  ...
  const el =
    ...
    // 地點名稱與標籤的對應字典 (Mapping)
    const siteTagsMapping = {
        "瘟豬壩沉墟": "sunken,ruin",          // 沉, 殘(墟)
        "電臺路焦土": "scorched,desolate,land", // 焦, 荒(土), 土地
        "山葬灰脈": "mountain,ash",           // 山, 灰
        "琉棘庭": "hall,desolate",            // 殿(庭), 荒
        "裂翼坪": "crack,plateau",            // 裂, 塬(坪)
        "軌畔孤構": "structure,desolate",     // 構, 荒(孤)
        "残柱林": "ruin,structure",           // 殘, 構(柱)
        "池骸灣": "remains,bay,water",        // 骸, 灣, 水(池)
        "褶層灣": "bay,crack",                // 灣, 裂(褶層)
        "釉骸拓壁": "remains,slope",          // 骸, 坡(壁)
        "疊骸構陣": "remains,structure",      // 骸, 構
        "苔網塬": "moss,web,plateau",         // 苔, 網, 塬
        "陸塢艦骸": "remains,port",           // 骸, 埠(塢)
        "墟響廳": "ruin,hall",                // 殘(墟), 殿(廳)
        "波蝕脊堤": "wave,eroded,shore",      // 波, 蝕, 岸(堤)
        "曜原驛": "plateau,structure",        // 塬(原), 構(驛)
        "溶境遺廊": "corridor,remains,water", // 廊, 骸(遺), 水(溶)
        "荒娛敖包": "desolate,structure",     // 荒, 構(敖包)
        "削巖殘居": "ruin,dwelling,mountain", // 殘, 居, 山(巖)
        "彩殼堡": "fort",                     // 堡
        "遷痕空埠": "relocated,port",         // 遷, 埠
        "山骸窟殿": "mountain,remains,hall"   // 山, 骸, 殿
    };
...
// 函數最結尾處
if (drawer) {
    drawer.setAttribute('data-tags', siteTags); // 綁定到最外層的 drawer
}
drawer.classList.add('open');
mask.classList.add('show');
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    indexTags.forEach(tag => {
        tag.addEventListener('click', (e) => {
            ...
            // 每次点击先重置所有标签的状态（实现单选逻辑）
            indexTags.forEach(t => t.classList.remove('active'));
            ...
            if (!isActive) {
                ...
                archiveDocs.forEach(doc => {
                    const docKeywords = doc.getAttribute('data-keywords') || "";
                    // 如果卡片的 data-keywords 包含当前选中的 keyword，则高亮；否则变暗
                ...
                // 如果点击的已经是激活状态的标签，则取消选中，并恢复所有卡片
                archiveDocs.forEach(doc => doc.classList.remove('dimmed'));
            }
        });
    });
});
/* =========================
   Archive Index Filtering System
========================= */
document.addEventListener('DOMContentLoaded', () => {
    const indexItems = document.querySelectorAll('.index-category, .index-tag');
    const archiveDocs = document.querySelectorAll('.archive-doc');
    ...
    function filterArchiveDocs() {
        // 获取当前所有被选中的关键词
        const activeElements = document.querySelectorAll('.index-category.active, .index-tag.active');
        const activeKeywords = Array.from(activeElements).map(el => el.getAttribute('data-keyword'));

        archiveDocs.forEach(doc => {
            // 如果没有任何标签被选中，显示所有卡片
            if (activeKeywords.length === 0) {
                doc.style.display = 'block'; // 或 'flex'，取决于你卡片原本的设定
                return;
            }

            // 获取当前卡片带有的 keywords（假设格式为 data-keywords="water,concrete,ruin"）
            const docKeywordsStr = doc.getAttribute('data-keywords');
            if (!docKeywordsStr) {
                doc.style.display = 'none';
                return;
            }

            const docKeywords = docKeywordsStr.split(',').map(k => k.trim());

            // 检查卡片是否包含【任意一个】被选中的关键词
            const hasMatch = activeKeywords.some(keyword => docKeywords.includes(keyword));

            if (hasMatch) {
                doc.style.display = 'block'; // 符合条件的显示
            } else {
                doc.style.display = 'none';  // 不符合的隐藏
            }
        });
    }
});