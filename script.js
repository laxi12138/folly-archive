...
const siteTagsMapping = {
    ...
    "土還灶垣": "slope, soil, eroded, wall",
    "崖倉蝕垣": "soil, mountain, slope, wall, eroded"
};
    ...
    },
{
  ...
  type: "record"
    ...
    {
        name: "土還灶垣",
        desc: "據說這片夯土殘牆曾是一座寺院的廚房。風雨沿著夯層逐年削去牆體，稜角變鈍，泥土與碎石重新顯露。這些牆原本從山土中一層層夯築而成，如今又一層層剝落回到土地，遠看時已逐漸分不清是建築正在消失，還是山體正在將它收回。",
        lat: 31.6644440,
        lng: 99.6794440,
        archiveDate: "2026.08",
        recorder: "王一川",
    ...
    {
        name: "崖倉蝕垣",
        desc: "這處夯土殘構高懸於坡崖之上，牆面一列列孔洞像是木梁曾經穿入後留下的骨位。從形制與位置判斷，它或許曾是一座依坡而築的倉房，也可能兼作守望之所：糧食、柴薪與日常之物被抬上高處，遠離潮濕與獸擾，同時俯視著山下的聚落。如今木構早已朽盡，只剩厚重的土牆仍嵌在山體邊緣，任風雨沿著梁孔與裂縫持續掏空它，彷彿整座房子正被山坡緩慢吞回。",
        lat: 31.6727778,
        lng: 99.6750000,
        archiveDate: "2026.08",
        recorder: "王一川",
...
function buildFileStacks() {
    ...
    function renderStack(siteArray, container, isGarden) {
        ...
        siteArray.forEach((site, index) => {
            ...
            const recorderName = site.recorder || '羅清源';
            const creatorMetaHtml = isGarden
                ? `<span data-i18n="ui_creator">墟構師: 羅清源</span>`
                : `<span data-i18n="ui_recorder_label">記錄者: </span><span class="doc-recorder-name">${recorderName}</span>`;
...
function switchLanguage(targetLang) {
    ...
    document.documentElement.lang = targetLang === 'ja' ? 'ja' : targetLang === 'en' ? 'en' : 'zh-Hant';
