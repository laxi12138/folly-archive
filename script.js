...
function openDrawerByIndex(i) {
    ...
    if (typeof window.hideCompass === 'function') {
        window.hideCompass();
        const collapseBtn = document.getElementById('collapse-compass-btn');
...
window.showCompass = function () {
    ...
    if (typeof bounds !== 'undefined') {
        map.flyToBounds(bounds, {
            animate: true,
            duration: 2.5,      // 缓缓拉回的时长
            easeLinearity: 0.1  // 保持丝滑的缓动起停
        });
    }

    // 🌟 2. 将罗盘放置在 main-viewport-frame 的正中心
    ...
    if (mainFrame && compassContainer) {
        ...
        compassX = frameRect.left + frameRect.width / 2;
        compassY = frameRect.top + frameRect.height / 2;
...
function buildFileStacks() {
    ...
    if (!collapseBtn) {
        ...
        // 👇 新增這行：將按鈕綁定到翻譯系統的 key
        collapseBtn.setAttribute('data-i18n', 'ui_collapse_compass');
