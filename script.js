...
function renderPage(num) {
    ...
    renderingDoc.getPage(num).then(function (page) {
        ...
        if (translationCanvas) {
            translationCanvas.width = viewport.width;
            translationCanvas.height = viewport.height;
            translationCanvas.getContext('2d')?.clearRect(0, 0, viewport.width, viewport.height);
        }
...
function clearInlineDocumentTranslation() {
    clearPdfTranslationCanvas();
    const content = document.getElementById('archive-text-content');
    if (content && activeAttachmentItem?.mode === 'text') {
        content.classList.remove('is-translating', 'is-translated');
        if (activeTextSource) content.textContent = activeTextSource;
    }
}
...
function paintPdfTranslationBlock(ctx, block, translated, targetLang, allBlocks, canvasWidth, canvasHeight) {
    ...
    ctx.fillStyle = 'rgba(22,22,22,.94)';
...
async function translatePdfBlocksInline(targetLang, requestToken) {
    const canvas = document.getElementById('pdf-translation-canvas');
    if (!canvas || !activePdfTextBlocks.length || targetLang === 'zh') {
        clearPdfTranslationCanvas();
        return;
    }
    ...
    async function worker() {
        while (cursor < blocks.length) {
            ...
            try {
                ...
                if (requestToken !== documentTranslationToken || activeAttachmentItem?.mode !== 'pdf') return;
...
async function translateTxtInline(targetLang, requestToken) {
    const content = document.getElementById('archive-text-content');
    if (!content || !activeTextSource) return;

    if (targetLang === 'zh') {
        content.classList.remove('is-translating', 'is-translated');
        content.textContent = activeTextSource;
        return;
    }

    content.classList.add('is-translating');
    content.classList.remove('is-translated');

    try {
        const translated = await translateDocumentText(activeTextSource, 'zh', targetLang);
        if (requestToken !== documentTranslationToken || activeAttachmentItem?.mode !== 'text') return;
        content.textContent = translated;
        content.classList.remove('is-translating');
        content.classList.add('is-translated');
    } catch (error) {
...
function refreshInlineDocumentTranslation() {
    ...
    // Simplified Chinese is the archival source. In Chinese UI mode the translation
    // system stays completely off and the untouched original is shown.
    if (targetLang === 'zh') {
        clearInlineDocumentTranslation();
        return;
    }
