...
let activePdfPageSource = '';
let documentTranslationToken = 0;
let documentTranslationVisible = true;
    ...
    if (item.mode === 'text') {
        ...
        fetch(item.src)
            ...
            .then(text => {
                ...
                refreshActiveDocumentTranslation();
            })
            .catch(error => {
...
function renderPage(num) {
    ...
    renderingDoc.getPage(num).then(function (page) {
        ...
        if (viewport.width * viewport.height > 15000000) {
            console.warn('PDF page is very large; using reduced render scale.');
            currentScale = 1.0;
        ...
        return Promise.all([renderTask.promise, textPromise]).then(([, textContent]) => {
            ...
            // If the user skipped ahead while this page was rendering,
            // do not waste an AI request translating the intermediate page.
            if (pending === null || pending === num) {
                if (textContent && Array.isArray(textContent.items)) {
                    activePdfPageSource = pdfTextContentToReadableText(textContent.items);
                    refreshActiveDocumentTranslation();
                } else {
                    activePdfPageSource = '';
                    showDocumentTranslationMessage(getDocumentTranslationUi().noText);
                }
            }
        ...
        pageNumPending = null;
        updatePdfHudState();
        showDocumentTranslationMessage(getDocumentTranslationUi().error);
    });
}
...
// =========================
// PDF / TXT live translation layer
// =========================
const DOCUMENT_TRANSLATION_UI = {
    zh: {
        title: '自动解译',
        loading: '正在解译当前文档…',
        unavailable: '翻译服务暂不可用。',
        noText: '当前 PDF 页面没有可提取的文字层。',
        error: '文档解译失败。'
    },
    en: {
        title: 'AUTO TRANSLATION',
        loading: 'Translating current document…',
        unavailable: 'Translation service is unavailable.',
        noText: 'No extractable text layer was found on this PDF page.',
        error: 'Document translation failed.'
    },
    ja: {
        title: '自動翻訳',
        loading: '現在の文書を翻訳中…',
        unavailable: '翻訳サービスを利用できません。',
        noText: 'このPDFページには抽出可能なテキストレイヤーがありません。',
        error: '文書の翻訳に失敗しました。'
    }
};

function getDocumentTranslationUi() {
    return DOCUMENT_TRANSLATION_UI[window.currentLang] || DOCUMENT_TRANSLATION_UI.zh;
}

function ensureDocumentTranslationPanel() {
    let panel = document.getElementById('document-translation-panel');
    if (panel) return panel;

    panel = document.createElement('aside');
    panel.id = 'document-translation-panel';
    panel.className = 'document-translation-panel';
    panel.hidden = true;
    panel.innerHTML = `
        <div class="document-translation-head">
            <span id="document-translation-title"></span>
            <button id="document-translation-toggle" type="button" aria-pressed="true">◫</button>
        </div>
        <div id="document-translation-meta" class="document-translation-meta"></div>
        <div id="document-translation-content" class="document-translation-content"></div>
    `;

    const sheet = document.querySelector('.attachment-sheet');
    if (sheet) sheet.appendChild(panel);

    panel.querySelector('#document-translation-toggle')?.addEventListener('click', event => {
        event.stopPropagation();
        documentTranslationVisible = !documentTranslationVisible;
        panel.classList.toggle('collapsed', !documentTranslationVisible);
        event.currentTarget.setAttribute('aria-pressed', documentTranslationVisible ? 'true' : 'false');
    });

    return panel;
}

function hideDocumentTranslationPanel() {
    const panel = document.getElementById('document-translation-panel');
    if (panel) panel.hidden = true;
}

function showDocumentTranslationMessage(message) {
    if (!activeAttachmentItem || !['pdf', 'text'].includes(activeAttachmentItem.mode)) return;

    const panel = ensureDocumentTranslationPanel();
    if (!panel) return;

    const ui = getDocumentTranslationUi();
    panel.hidden = false;
    panel.classList.toggle('collapsed', !documentTranslationVisible);

    const title = panel.querySelector('#document-translation-title');
    const meta = panel.querySelector('#document-translation-meta');
    const content = panel.querySelector('#document-translation-content');
...
function detectDocumentLanguage() {
    // Kept as a compatibility helper. Archive document masters are always zh.
    return 'zh';
}
...
function pdfTextContentToReadableText(items) {
...
async function refreshActiveDocumentTranslation() {
    const item = activeAttachmentItem;
    if (!item || !['pdf', 'text'].includes(item.mode)) return;

    const sourceText = item.mode === 'pdf'
        ? activePdfPageSource
        : activeTextSource;
    ...
    const ui = getDocumentTranslationUi();
    const targetLang = ['zh', 'en', 'ja'].includes(window.currentLang) ? window.currentLang : 'zh';
    // Archive documents use Simplified Chinese as the canonical source language.
    // Do not auto-detect: this prevents stale/short samples from becoming JA → JA or EN → EN.
    const sourceLang = 'zh';
    ...
    if (title) title.textContent = ui.title;
    if (meta) {
        meta.textContent = `${sourceLang.toUpperCase()} → ${targetLang.toUpperCase()} · ${
            item.mode === 'pdf' ? `P.${pageNum}` : 'TXT'
        }`;
    }

    if (sourceLang === targetLang) {
        if (content) content.textContent = sourceText;
        return;
    }

    if (content) content.textContent = ui.loading;

    try {
        const translated = await translateDocumentText(sourceText, sourceLang, targetLang);
        if (requestToken !== documentTranslationToken || activeAttachmentItem !== item) return;

        if (content) {
            if (typeof cyberDecodeTranslate === 'function') {
                cyberDecodeTranslate(content, translated, 650);
            } else {
        ...
        console.warn('Document translation unavailable:', error);
        if (content) content.textContent = ui.unavailable;
    }
}

window.handleDocumentLanguageChange = function handleDocumentLanguageChange() {
    if (activeAttachmentItem && ['pdf', 'text'].includes(activeAttachmentItem.mode)) {
        refreshActiveDocumentTranslation();
    }
};
