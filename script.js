...
function detectDocumentLanguage(text) {
    ...
    const han = (sample.match(/[一-龯]/g) || []).length;
    const latin = (sample.match(/[A-Za-z]/g) || []).length;

    if (han > Math.max(8, latin * 0.25)) return 'zh';
    return 'en';
}
...
async function refreshActiveDocumentTranslation() {
    ...
    const ui = getDocumentTranslationUi();
    const targetLang = window.currentLang || 'zh';
    const sourceLang = item.sourceLang || detectDocumentLanguage(sourceText);
