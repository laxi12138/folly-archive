...
<div class="tree-branch tree-folder" onclick="toggleFolder('${folderId}')">
  ...
  <span data-i18n="${labelKey}"></span> (${count})
</div>
...
function updateRecordNav() {
    ...
    if (isOpen) {
        ...
        recordLinkEl.innerHTML = `<span style="font-weight: 300; margin-right: 12px; display: inline-block;">➢</span>[ <span data-i18n="ui_record">遺構錄</span> | ${site.name} ]`;
    } else {
        // 2. 关闭状态下：移除激活类名，恢复原样，把控制权交回给 CSS hover
        recordLinkEl.classList.remove('compass-active');
        recordLinkEl.innerHTML = `[ <span data-i18n="ui_record">遺構錄</span> | ${site.name} ]`;
    }
...
function buildFileStacks() {
    ...
    function renderStack(siteArray, container, isGarden) {
        ...
        siteArray.forEach((site, index) => {
            ...
            const navKey = isGarden ? 'ui_auto_nav' : 'ui_compass_nav';
            const navText = isGarden ? '自動導航 ⌖' : '⊙ 導航羅盤';

            const titleTextHtml = isGarden
                ? `<span data-i18n="ui_garden">廢墟園林</span> · 其${seq} | ${site.name}`
                : `<span data-i18n="ui_record">遺構錄</span> | ${site.name}`;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function updateArchiveDocs() {
        ...
        // B. 获取页面上的所有卡片和地图字符节点
        const archiveDocs = document.querySelectorAll('.archive-doc, .site-character');
...
if (ruinTrigger && recordTrigger) {
    // 如果没有任何选中的标签，恢复默认的初始文本
    if (activeTags.length === 0) {
        ruinTrigger.textContent = "廢墟園林系列";
        recordTrigger.textContent = "遺構錄系列";
    } else {
        // 1. 初始化计数器
        let gardenCount = 0;
        let recordCount = 0;
        ...
        // 3. 生成圆点字符串，例如计数为 3，则生成 "• • •"
        const gardenDots = Array(gardenCount).fill("•").join(" ");
        const recordDots = Array(recordCount).fill("•").join(" ");

        // 4. 更新 DOM 文本 (巧妙利用 innerHTML 分離圓點與翻譯標籤)
        ruinTrigger.innerHTML = (gardenCount > 0 ? `<span style="letter-spacing: 2px; opacity: 0.8;">${gardenDots}</span> ` : "") + `<span data-i18n="bottom_trigger_ruin">⁙廢墟園林系列</span>`;

        recordTrigger.innerHTML = `<span data-i18n="bottom_trigger_record">遺構錄系列</span>` + (recordCount > 0 ? ` <span style="letter-spacing: 2px; opacity: 0.8;">${recordDots}</span>` : "");
