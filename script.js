...
// 每次打开附件前，确保重置翻页 HUD 显示状态
    ...
    if (item.mode === 'pdf') {
        // 🌟 加载状态与错误显示层
        wrapper.innerHTML = `
            <div id="pdf-loading" style="position: absolute; font-size: 13px; opacity: 0.6; letter-spacing: 2px;">讀取圖紙中...</div>
            <canvas id="pdf-canvas" class="attachment-image" style="opacity: 0; transition: opacity 0.4s ease;"></canvas>
        `;
        ...
        window.pdfjsLib.getDocument(item.src).promise.then(function (pdf) {
            pdfDoc = pdf;
            document.getElementById('pdf-page-num').innerText = `${pageNum} / ${pdfDoc.numPages}`;
            renderPage(pageNum);
        }).catch(function(error) {
            console.error('PDF.js Error:', error);
            // 如果被浏览器安全机制拦截，直接把红字打印在屏幕中心
            wrapper.innerHTML = `
                <div style="color: rgba(0,0,0,0.6); font-size: 13px; text-align: center; line-height: 2;">
                    <b>圖紙渲染失敗</b><br>
                    <span style="opacity: 0.7;">${error.message}</span><br><br>
                    ⚠️ 如果您正使用 file:/// 預覽，請改用 Live Server 以解除瀏覽器本地安全限制。
                </div>`;
        });
    }
  if (item.mode === 'video') {
