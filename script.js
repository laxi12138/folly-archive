...
<div
  ...
  <div class="tree-collapse archive-record-subcollapse">
    ...
    <div
      class="tree-file archive-record-file"
      onclick="openAttachmentViewer('radio-note-2')">
  ...
  <div class="tree-collapse archive-record-subcollapse">
    ...
    <div
      class="tree-file archive-record-file"
      onclick="openAttachmentViewer('radio-map-2')">
...
{
    name: "噬山窯場",
    desc: "水泥廠依附於被開採過的山體邊緣而建。當生產停止後，留下的不只是廢棄設施，還有一座失去半邊輪廓的石灰岩山。",
  lat: 32.04174,
  lng: 119.83912,
  archiveDate: "2017.08",
  type: "record"
    ...
    <div class="archive-popup">
      <div class="archive-content">
        ...
        <div class="archive-drawer-link" onclick="window.openDrawerByIndex(${index}, this)">
          <span class="label">${site.type === "garden" ? "廢墟劇場" : "遺構錄"}</span>
        </div>
      </div>
    </div>
  `, {
...
window.updateCompassDirection = function () {
    ...
    // CSS 中 .compass-ring 旋转了 45 度
    const rotationAngleDeg = 45;
    const rotationAngleRad = rotationAngleDeg * (Math.PI / 180);

    // 极其关键：因为箭头在 compass-ring 内部，它继承了 45 度旋转。
    // 所以我们求出相对于这 45 度的【局部夹角】
    const localAngleRad = globalAngleRad - rotationAngleRad;

    // ⚠️ 修复点：必须使用 offsetWidth/offsetHeight 才能拿到椭圆真实的像素尺寸
    // 之前用的 ringRect 包含旋转后的外框，导致宽高膨胀，所以箭头不贴边
    const a = compassRing.offsetWidth / 2;
    const b = compassRing.offsetHeight / 2;

    const cosT = Math.cos(localAngleRad);
    const sinT = Math.sin(localAngleRad);

    // 计算在该局部角度下，椭圆边缘的精准半径 R
    const radiusAtAngle = (a * b) / Math.sqrt(Math.pow(b * cosT, 2) + Math.pow(a * sinT, 2));

    // 计算箭头在局部坐标系中的 X, Y 偏移
    const localX = radiusAtAngle * cosT;
    const localY = radiusAtAngle * sinT;

    // 箭头的朝向角度：屏幕全局角度 + 90度(修正文字↑朝上) - 45度(抵消父级旋转)
    let arrowRotDeg = (globalAngleRad * 180 / Math.PI) + 90 - rotationAngleDeg;

    // ==========================================
    // 渲染动作指令
    // ==========================================
    if (distanceToTarget > radiusAtAngle) {
        // 目标在圈外：朝向圈外
        arrow.style.transform = `translate(${localX}px, ${localY}px) rotate(${arrowRotDeg}deg)`;
    } else {
        // 目标在圈内：调头180°朝向圈内
        arrow.style.transform = `translate(${localX}px, ${localY}px) rotate(${arrowRotDeg + 180}deg)`;
    }
};
/* =========================
...
document.addEventListener('click', (e) => {
    if (e.target.closest('#main-title')) return;
    if (mainTitle) {
        mainTitle.classList.add('hidden');
        mainTitle.style.pointerEvents = 'none';
    }
