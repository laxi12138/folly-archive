...
const RECORD_STACK_STEP_MS = 145;
...
function layoutRecordStackSlider() {
    ...
    // The diagonal fan itself moves a little as the archive window travels,
    // so the motion reads as sliding a physical stack rather than swapping rows.
    ...
    state.docs.forEach((doc, index) => {
        ...
        if (index > end) {
            // Newer documents are pressed flat above the current fan.
            const distance = index - end;
            top = activeTop - distance * topGap;
            left = 0;
            z = 900 + Math.max(0, topCount - distance);
            doc.classList.add('stack-flat-top');
        } else if (index < start) {
            // Older documents are pressed flat below the current fan and align
            // to the bottom-most diagonal card, matching the user's mock-up.
            const distance = start - index;
            top = activeBottom + distance * bottomGap;
            left = bottomFanX;
            z = 1000 + activeCount + distance;
            doc.classList.add('stack-flat-bottom');
        } else {
            // Original data order is old -> new. The newest visible record sits
            // at the top of the fan, so rank counts backwards from windowEnd.
            const rank = end - index;
            top = activeTop + rank * RECORD_STACK_FAN_GAP_Y;
            left = -rank * RECORD_STACK_FAN_GAP_X;
            z = 1000 + rank;
            doc.classList.add('stack-fan');
        }
...
function shiftRecordStackWindow(direction) {
    ...
    if (document.querySelector('#stack-record .archive-doc.extracted')) return false;
...
function setRecordStackHoverDirection(direction) {
    ...
    // One immediate, gentle step makes the interaction feel responsive.
    shiftRecordStackWindow(direction);
    state.hoverTimer = setInterval(() => {
        if (!shiftRecordStackWindow(direction)) stopRecordStackHover();
    }, RECORD_STACK_STEP_MS);
}

function handleRecordStackPointerMove(event) {
    const state = recordStackSlider;
    if (!state.container || !state.docs.length || isCompactViewport()) {
        stopRecordStackHover();
        return;
    }
    if (event.pointerType && event.pointerType !== 'mouse' && event.pointerType !== 'pen') return;
    if (document.querySelector('#stack-record .archive-doc.extracted')) {
        stopRecordStackHover();
        return;
    }
    ...
    // Hover anywhere over the flattened upper/lower files, or close to the
    // corresponding edge of the diagonal fan, to slide that way.
    if (
        state.windowEnd < state.total - 1 &&
        y <= state.activeTop + RECORD_STACK_HOVER_EDGE
    ) {
    ...
    if (
        state.windowStart > 0 &&
        y >= state.activeBottom - RECORD_STACK_HOVER_EDGE
    ) {
        setRecordStackHoverDirection(-1);
        return;
    }
    ...
    function renderStack(entries, container, isGarden) {
        ...
        entries.forEach((entry, index) => {
            ...
            docEl.addEventListener('click', (event) => {
                ...
                const willExtract = !docEl.classList.contains('extracted');
                if (willExtract && !isGarden) {
                    const stackRect = container.getBoundingClientRect();
                    const docRect = docEl.getBoundingClientRect();
                    const targetBottom = 46;
                    const targetViewportTop = Math.max(18, window.innerHeight - docRect.height - targetBottom);
                    const targetRelativeTop = targetViewportTop - stackRect.top;
                    docEl.style.setProperty('--record-extract-top', `${targetRelativeTop}px`);
                }

                docEl.classList.toggle('extracted');
                docEl.style.zIndex = docEl.classList.contains('extracted')
                    ? '5000'
                    : docEl.dataset.zIndex;
