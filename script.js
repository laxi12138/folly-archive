    ...
    if (indexDrawer && triggers.length > 0) {
        triggers.forEach(trigger => {
            trigger.addEventListener('click', (e) => {
                ...
                // 🌟 新增：只适用于竖屏手机版（max-width: 768px）的专属侧菜单绑定逻辑
                if (window.innerWidth <= 768) {
                    ...
                    if (trigId === 'bottom-trigger-record' && drawerLeft) {
                        ...
                        return; // 拦截后续逻辑，不打开 index-drawer
                    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function buildMobileLists() {
        ...
        const renderList = (siteArray, container) => {
        ...
        renderList(recordSites, leftList); // 遗构录在左边
        renderList(gardenSites, rightList); // 废墟园林在右边
    }
