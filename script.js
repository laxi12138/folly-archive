...
if (item.mode === 'card') {
 ...
 if (item.mode === 'text') {
  ...
  `;
    ...
    if (item.mode === 'pdf') {
        // 用 Canvas 替代原生的 embed
        wrapper.innerHTML = `<canvas id="pdf-canvas" class="attachment-image"></canvas>`;

        if (pdfHud) pdfHud.style.display = 'flex'; // 显示翻页按钮
...
function renderPage(num) {
    ...
    pdfDoc.getPage(num).then(function (page) {
        ...
        if (!canvas) return; // 如果用户在渲染完成前关闭了窗口
