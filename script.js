...
/* =========================
   Archive Index Filtering System (严格多选/交集匹配)
========================= */
document.addEventListener("DOMContentLoaded", () => {
    // 1. 获取所有的索引标签和分类标签
    ...
    indexTags.forEach(tag => {
        tag.addEventListener('click', (e) => {
            e.stopPropagation(); // 防止点击标签时，触发 document 的关闭抽屉事件

            // 切换该标签的 active 状态（允许多个同时存在，实现多选）
    ...
    function updateArchiveDocs() {
        // 收集当前所有被选中（带 'active' 类）的标签的 data-tag 值
        ...
        // 遍历所有档案卡片（或地图元素）进行多选匹配
        archiveDocs.forEach(doc => {
            // 兼容你代码中出现的 data-tag 和 data-tags 两种属性写法
            const docTagsAttr = doc.getAttribute('data-tag') || doc.getAttribute('data-tags') || "";

            // 将字符串切割成数组，并去除多余空格（例如 "ruin, moss, tower"）
            const docTags = docTagsAttr.split(',').map(t => t.trim());

            // 如果没有选中任何标签，恢复初始状态
            if (activeTags.length === 0) {
                doc.classList.remove('matched-tag');
                doc.classList.remove('active-dot');
                doc.style.display = 'block'; // 恢复显示
                return;
            }

            // 🔥 关键修改点：使用 .every() 代替 .some()
            // .every() 表示：卡片自带的 docTags 必须包含当前选中的 activeTags 中的【每一个】，才会返回 true
            const isMatch = activeTags.every(activeTag => docTags.includes(activeTag));

            if (isMatch) {
                // 完全符合所有选中的标签
                doc.classList.add('matched-tag');
                doc.classList.add('active-dot'); // 触发黑点
                doc.style.display = 'block';     // 确保显示
            } else {
                // 不符合条件（被排除）
                doc.classList.remove('matched-tag');
                doc.classList.remove('active-dot'); // 移除黑点
