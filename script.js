...
function animateCompassPhysics() {
    ...
    if (ring) ring.style.transform = `rotate(${currentRingAngle}deg)`;
    const asterisk =
        document.querySelector(
            '.compass-asterisk'
        );
    ...
    if (ring) {
        ...
        const a = ring.offsetWidth / 2;
        const b = ring.offsetHeight / 2;
        ...
        const radius = (a * b) / Math.sqrt(Math.pow(b * Math.cos(phi), 2) + Math.pow(a * Math.sin(phi), 2));
