  ...
  chapterToggle.addEventListener("click", () => {
    ...
    if (scoreHUD) {
        ...
        scoreHUD.addEventListener('mousemove', (e) => {
            ...
            // 动态让顶层发生位移
            scoreHUD.style.setProperty('transform', `translate(${moveX}px, ${moveY}px)`, 'important');
        });

        // 2. 监听鼠标移出区域，触发缓慢回弹
        scoreHUD.addEventListener('mouseleave', () => {
            // 鼠标离开时，重新恢复 0.8s 的 cubic-bezier 缓动时间，实现丝滑回弹
            scoreHUD.style.setProperty('transition', 'opacity 1.2s ease-out, transform 0.8s cubic-bezier(0.25, 1, 0.5, 1)', 'important');

            // 归位到原点 (0, 0)
            scoreHUD.style.setProperty('transform', 'translate(0px, 0px)', 'important');
        });
    }
  /* >>>>>>>>>>>>>>> 新代码插入结束 >>>>>>>>>>>>>>> */
  if (scoreHUD) {
