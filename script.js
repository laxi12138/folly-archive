...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function bringDrawerToFront(element) {
        ...
        function toggleIndexDrawerWithAnim() {
            ...
            if (!isOpen) {
                ...
                // 3. 等待抽屜完全打開 (400ms) 後，Z軸躍升，並從下方拉回文件疊
                setTimeout(() => {
                    stacks.forEach(s => {
    ...
    function closeIndexDrawerWithAnim(skipRestore = false) {
        ...
        // 3. 等待收回完畢 (400ms) 後，若未跳過則恢復原本的 Z 軸，並重新升起原版文件疊
        if (!skipRestore) {
            setTimeout(() => {
                stacks.forEach(s => {
    ...
    // 3. 禁止键盘快捷键复制（如 Ctrl+C / Cmd+C、Ctrl+A 等）
...
if (chapterToggle && videoChapters) {
  ...
  chapterToggle.addEventListener("click", () => {
    ...
    if (scoreHUD) {
        const maxMovePx = 15;
        let hudMoveRaf = null; // 声明动画帧锁

        // 🌟 新增：用來記錄上一次滑鼠座標，作為過濾顫動的基準
        let lastMouseX = 0;
        let lastMouseY = 0;
        ...
        scoreHUD.addEventListener('mousemove', (e) => {
            ...
            // 🌟 核心優化：座標變更閾值。如果移動距離不到 2px，直接拋棄，不觸發重繪
            ...
            hudMoveRaf = requestAnimationFrame(() => {
                ...
                hudMoveRaf = null; // 任务执行完毕，释放锁！
            });
        }); // <--- mousemove 监听结束

        // 2. 监听鼠标离开（必须与 mousemove 平级，不要包在里面！）
...
/* =========================
   动态溶解薄膜效果 (Zoom 响应 - 极限性能优化版)
========================= */
let membraneZoomRaf = null; // 声明动画帧锁

function updateMembraneEffect() {
    // 节流核心：如果当前有一帧在排队，直接抛弃本次事件
    if (membraneZoomRaf) return;

    membraneZoomRaf = requestAnimationFrame(() => {
        const el = overlay.getElement();
        if (!el) {
            membraneZoomRaf = null;
            return;
        }
        ...
        if (currentZoom <= triggerZoom) {
            // 基础状态保持不变
            el.style.filter = 'blur(0.4px) contrast(1.8) brightness(1.02) sepia(0.33)';
            el.style.opacity = 1;
            el.style.mixBlendMode = 'normal';
        } else {
            const dissolveRatio = Math.min((currentZoom - triggerZoom) / (maxZoom - triggerZoom), 1);

            // 1. 降低 blur 的动态极值（原为加上 3.5，现降低为加上 1.2）
            // 因为在巨大的 SVG 上大范围模糊是引发卡顿的次要原因
            const dynamicBlur = 0.40 + (dissolveRatio * 2.3);
            const dynamicContrast = 1.8 - (dissolveRatio * 0.8);
            const dynamicBrightness = 1.02 + (dissolveRatio * 0.7);
            const dynamicInvert = dissolveRatio * 0.15;

            // 2. 🚨 核心优化：彻底移除 drop-shadow！
            // 仅保留 blur, contrast, brightness, sepia 和 invert，足以维持“溶解”的视觉错觉
            el.style.filter = `
                blur(${dynamicBlur}px) 
        ...
        membraneZoomRaf = null; // 任务执行完毕，释放锁
    });
}
...
function bindVideoUI() {
    ...
    // ✨ 核心优化：在 timeupdate 外部，提前声明尺寸缓存变量
    ...
    video.ontimeupdate = () => {
        ...
        if (viewer.classList.contains('score-linear')) {
            ...
            // ✨ 核心优化：懒加载缓存尺寸，避免每秒 60 次的 Layout Thrashing
...
// ✨ 新增：尺寸缓存变量，彻底消除 Layout Thrashing
...
function animateCompassPhysics() {
    ...
    const asterisk = document.querySelector('.compass-asterisk');
...
window.updateCompassDirection = function () {
    ...
    // ✅ 核心修复：直接使用全局维护的中心坐标，彻底消灭 Layout Thrashing！
    ...
    // ✨ 优化：顺手把这里的 offsetWidth 也替换成了缓存变量，彻底剿灭 Layout Thrashing
...
function flyToSite(site, index, fromIndexDrawer = false) {
    ...
    // 🌟 2. 刪除強制下沉的這一行，常規點擊不再下沉！
    // stacks.forEach(s => s.classList.add('sink-down')); 

    const finishFlyTo = () => {
        // 🌟 3. 只有當這場飛行是從打開的 index-drawer 觸發時，才需要在結束時解除下沉狀態
        if (fromIndexDrawer) {
            stacks.forEach(s => {
                s.classList.remove('sink-down');
                s.classList.remove('elevated-z');
    ...
    setTimeout(() => {
        ...
        map.once('moveend', () => {
            ...
            // 🌟 新增：彈出 Popup 且開始展開目錄時，顯示「正在打開檔案目錄」
            const dirHint = document.getElementById('map-dir-hint');
...
/* =========================
   Marker 透明度随 Zoom 缩放动态变淡 (性能优化版)
========================= */
let markerOpacityRaf = null; // 声明动画帧锁
...
if (joystick && knob) {
    ...
    document.addEventListener('pointermove', (e) => {
        ...
        // ✨ 核心优化：rAF 节流，挡掉多余的高频事件
        if (joyRaf) return;

        joyRaf = requestAnimationFrame(() => {
            // ✨ 核心优化：使用缓存的坐标，彻底消灭 Layout Thrashing
        ...
        siteArray.forEach((site, index) => {
            ...
            if (coordBtn) {
                coordBtn.addEventListener('click', (e) => {
                    ...
                    // 🌟 核心判断：检查 index-drawer 是否处于开启状态
                    const indexDrawer = document.getElementById('index-drawer');
                    if (indexDrawer && indexDrawer.classList.contains('open')) {
                        // 【情况 A】：当 index-drawer 已打开时，调用挂载到 window 上的全局函数
                        window.closeIndexDrawerWithAnim(true);

                        setTimeout(() => {
                            // 🌟 傳遞 true：告訴 flyToSite 這是從抽屜觸發的
                            if (typeof flyToSite === 'function') flyToSite(site, originalIndex, true);
                        }, 400);
                    } else {
                        // 【情况 B】：当 index-drawer 未打开时，直接启动飞行动画
                        // 🌟 傳遞 false：常規點擊，不要下沉
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function updateArchiveDocs() {
        ...
        /* ===================================================
           E. 新增：动态更新底部标签文本，显示筛选结果的数量（数字）
           =================================================== */
        // 1. 获取所有新旧两处的触发器节点（支持批量查询）
        ...
        if (activeTags.length === 0) {
            ...
            // 3. 批量更新所有数字容器（同时作用于原始抽屉和打开后的底部装饰栏）
            ruinTriggers.forEach(trigger => {
                const dots = trigger.querySelector('.filter-dots');
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 3. 建议加上 ResizeObserver 以防相框尺寸发生动态变化
